...
#if defined(_MSC_VER)
  #define ATTR_ALIGNED16 __declspec(align(16))
#else
  #define ATTR_ALIGNED16 __attribute__((aligned(16)))
#endif
...
bool Secp256K1::EC(Point& p) {
    ...
    _s.ModMulK1(&p.y, &p.y);
    _s.ModSub(&_p);
    return _s.IsZero(); // ((y^2 - (x^3 + 7)) % P) == 0
}

Point Secp256K1::ScalarMultiplication(Point& P, Int* scalar) {
    Point R, Q, T;
    int  no_of_bits = scalar->GetBitLength();
    R.Clear();
    R.z.SetInt32(1);
    if (!scalar->IsZero()) {
        Q.Set(P);
...
void Secp256K1::GetHash160(int type, bool compressed,
    ...
    ATTR_ALIGNED16 unsigned char sh0[64];
    ATTR_ALIGNED16 unsigned char sh1[64];
    ATTR_ALIGNED16 unsigned char sh2[64];
    ATTR_ALIGNED16 unsigned char sh3[64];
